...
class FragmentUserTimeline : BaseFragment() {
    ...
    private fun loadTweetWhenConnected() {
        ...
        val userTimeline = statusesService.userTimeline(twitterUser!!.userId, twitterUser.screenName, 200, null, null, null, null, null, true)
        userTimeline.enqueue(object : Callback<MutableList<Tweet>>() {
            override fun success(result: Result<MutableList<Tweet>>?) {
                TableTweetHelper.getInstance(context!!).deleteAllUserTimeline()
                tweetList = arrayListOf()
                tweetList.addAll(result!!.data)

                Thread(Runnable {
                    for (t in tweetList) {
                        TableTweetHelper.getInstance(context!!).insertUserTweet(t)
                    }
                }).start()
    ...
    private fun loadTweetWhenOffline() {
        ...
        tweetList = arrayListOf()
        tweetList = TableTweetHelper.getInstance(context!!).getUserTweetList().toMutableList()
        initRecyclerView(rvTweet!!, tweetList, context!!)
    }
}